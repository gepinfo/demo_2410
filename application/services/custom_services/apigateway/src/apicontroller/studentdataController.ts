...
this.router.get('/studentdata/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
        //#@gepbankingapiroute@#
...
public GpGetNounCreatedBy(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into studentdataController.ts: GpGetNounCreatedBy');
        new ApiAdapter().get(Constant.STUDENTDETAILSURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from studentdataController.ts: GpGetNounCreatedBy');
        }).catch(err => {
            res.send(err);
        });
    }
